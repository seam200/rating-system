...
if ($_SERVER['REQUEST_METHOD'] == "POST" && isset($_POST['rating']) == "done") {
    ...
    $selctEmailExist = $conn->query("SELECT * FROM `ratings` WHERE `gmail` = '$review_email'");
    ...
    }elseif($selctEmailExist->num_rows > 1){
        $res = array(
            "error" => 5,
            "error_message" => "This email owner already rate us!"
        );
        exit(json_encode($res));
